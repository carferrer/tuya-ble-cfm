...
- Passage mode switch (DP33) on `b3aouluh`, enabled as an entity by default
- Last connected and Update interval diagnostic sensors on both lock families

The Passage mode switch reflects the lock's reported DP33 state and only
accepts a change after the lock confirms it. Enabling the entity does not
automatically put the physical lock into passage mode. The Access event entity
also emits `passage_mode_enabled` (`event_type_id: 33`) when a reported DP33
changes from false to true. It does not emit when passage mode is turned off,
on an unchanged report, or from an already-open startup state. This event uses
the HA receipt time; it is separate from historical unlock records.

Last connected records the most recent successful paired BLE connection seen
by this HA process, even after the lock disconnects to save battery. It starts
unknown after an HA restart until a connection succeeds. Update interval shows
the configured periodic synchronization interval in minutes, or `0` for
on-demand, activity-detection and keep-alive modes. These diagnostic entities
do not make extra BLE connections.
...
record IDs: access types use their DP number (12, 13, 14, 15, 19, 33, 55, 62, 63),
