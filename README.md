...
En `b3aouluh`, una pulsación de `bluetooth_unlock` envía DP6 `true`, espera 500 ms tras completar esa escritura y envía DP6 `false`. Esto reproduce la doble pulsación que funcionó en la instalación, sin necesitar una automatización. Si falla la primera escritura, no envía la segunda. En `okkyfgfs`, solo el botón `bluetooth_unlock` prueba DP6 `raw` con valor `01 01` una sola vez por pulsación, sin pausa ni segundo envío. Se investiga así si una única orden tiene un comportamiento distinto respecto al paso libre; todavía no se ha validado físicamente esta variante. Las dos acciones de la entidad `lock` de `okkyfgfs` mantienen dos envíos de `01 01` separados por 500 ms, la secuencia que abrió la cerradura en la prueba física. Tanto el botón como la entidad `lock` pueden iniciar la conexión BLE cuando sea necesario.

La entidad `lock` de ambas familias muestra `locked` cuando el último DP47 Booleano recibido es `false` y `unlocked` cuando es `true`. Sin lectura previa, muestra `unknown`. Las acciones **Bloquear** y **Desbloquear** envían la secuencia doble de apertura de cada modelo: `true → 500 ms → false` en `b3aouluh` y `01 01 → 500 ms → 01 01` en `okkyfgfs`. Solo un nuevo DP47 puede actualizar el estado mostrado. Por tanto, **Bloquear** también envía una orden de apertura y no garantiza el cierre del modo de paso libre. En `b3aouluh`, el paso libre se controla con su interruptor específico; en `okkyfgfs` no se ha identificado un comando para ese modo. Como la cerradura se desconecta para ahorrar batería, el estado visible es la última lectura conocida y puede quedar desactualizado hasta recibir un nuevo DP47. En las pruebas de `okkyfgfs`, DP47 se recibe al cambiar el motor mientras está conectada, pero no se vuelve a enviar con cada actualización manual.
...
- Validar la nueva entidad `lock` de `okkyfgfs`, el experimento de un único envío DP6 `raw` y el alcance de la recuperación de registros al reconectar. El botón con doble envío, los eventos de apertura y varias notificaciones de alarma ya se han probado. No se asume que use DP69.
...
La suite actual contiene **128 pruebas**. Cubre el parser BLE, el alcance de los productos, las políticas de conexión, los IDs de entidades, el envío único desde el botón de `okkyfgfs`, las acciones dobles y estados de `lock` en ambas familias, la restauración de alarmas por cerradura y el comportamiento de los eventos: tipos, fechas, duplicados, carga inicial y recargas. También se comprueba con Ruff, Hassfest y HACS.
